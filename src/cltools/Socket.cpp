...
string Socket::description()const{ return "runs PLUMED over a socket"; }
...
int Socket::main(FILE* in,FILE*out,Communicator& pc){
  ...
  string timeunits="ps"; p.cmd("setMDTimeUnits", &timeunits);
  ...
  while (true) {
     ...
     if (strcmp(header,"STATUS      ") == 0 ) {
        ...
        p.cmd("calc",NULL); // do the actual calculation
...
PLUMED_REGISTER_CLTOOL(Socket,"socket")
