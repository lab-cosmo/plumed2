...
typedef struct {
  ...
  void(*grab_data)(void*,const char*,void*);
  void(*finalize)(void*);
...
extern "C" void plumedmain_grabdata(void*plumed,const char*key,void*val){
  plumed_massert(plumed,"trying to use a plumed object which is not initialized");
  static_cast<PLMD::PlumedMain*>(plumed)->grab_data(key,val);
}
...
static class PlumedMainInitializer{
  ...
  PlumedMainInitializer(){
    plumed_plumedmain_function_holder fh={plumedmain_create,plumedmain_cmd,plumedmain_grabdimension,plumedmain_grabdata,plumedmain_finalize};
